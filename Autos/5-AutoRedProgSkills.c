...
//From Hanging Zone, grab easy 2, stash them
	...
	auto(enc1(-1300),	0,		lPre(BMP),	IN,	0,	0,	DRIV_READY,	NEXT);	//Rev over bump
	auto(straight(REV),	0,		lPre(STS),	IN,	0,	0,	TIME_LIMIT,	500);	//Rev + lift
	auto(straight(REV),	0,		0,			0,	0,	0,	TIME_LIMIT,	800);	//Rev, HIT LARGE BALL 1
...
//Go and throw a large ball
	...
	auto(gyro2(-184),	0,		lPre(GND),	0,	0,	0,	DRIV_READY,	PID);	//Turn + drop
	auto(enc1(2300),	0,		lPre(GND),	0,	0,	0,	DRIV_READY,	NEXT);	//Forward past
	auto(stopped(),		LEFT,	lPre(GND),	OUT,0,	0,	TIME_LIMIT,	300);	//Forward into large
	auto(straight(60),	0,		lPre(GND),	OUT,0,	0,	TIME_LIMIT,	1450);	//Forward into large
	auto(enc1(-150),	0,		lPre(GND),	0,	0,	0,	DRIV_READY,	NEXT);	//Reverse
	auto(stopped(),encStrf1(-550),lPre(GND),0,	0,	0,	DRIV_READY,	NEXT);	//Strafe
	auto(enc1Spd(-1450,90),0,	lPre(GND),	0,	0,	0,	DRIV_READY,	NEXT);	//Reverse
	auto(enc1(-350),	0,		lPre(GND),	0,	1,	0,	DRIV_READY,	NEXT);	//REV + THROW
	auto(enc1(400),		0,		lPre(GND),	0,	1,	0,	DRIV_READY,	NEXT);	//Fwd
...
//From Middle Zone, grab 3
	...
	auto(enc1(120),		0,		lPre(GND),	0,	0,	0,	DRIV_READY,	NEXT);	//Fwd from ball
	auto(gyro2(180),	0,		lPre(STS),	0,	0,	0,	DRIV_READY,	PID);	//Turn + lift
	auto(stopped(),usStrfR(74),	lPre(STS),	0,	0,	0,	DRIV_READY,	NEXT);	//Strafe
	auto(straight(100),	0,		lPre(STS),	0,	0,	0,	TIME_LIMIT,	1100);	//Fwd
	auto(straight(-100),0,		lPre(STS),	0,	0,	0,	TIME_LIMIT,	200);	//Rev
...
//Go and throw a large ball
	auto(enc1(-320),	0,		lPre(STS),	OUT,0,	0,	DRIV_READY,	NEXT);	//Reverse
	auto(gyro2(184),	0,		lPre(GND),	0,	0,	0,	DRIV_READY,	PID);	//Turn + drop
	auto(enc1(2600),	0,		lPre(GND),	0,	0,	0,	DRIV_READY,	NEXT);	//Forward past
	auto(straight(60),	0,		lPre(GND),	OUT,0,	0,	TIME_LIMIT,	1200);	//Forward into large
	auto(enc1(-150),	0,		lPre(GND),	0,	0,	0,	DRIV_READY,	NEXT);	//Reverse
	auto(stopped(),encStrf1(550),lPre(GND),	0,	0,	0,	DRIV_READY,	NEXT);	//Strafe
	auto(enc1Spd(-1450,90),0,		lPre(GND),	0,	0,	0,	DRIV_READY,	NEXT);	//Reverse
	auto(enc1(-350),	0,		lPre(GND),	0,	1,	0,	DRIV_READY,	NEXT);	//REV + THROW
