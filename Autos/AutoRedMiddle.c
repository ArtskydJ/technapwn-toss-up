...
void autoRedMid2(void)
	...
	auto(gyro2(25),		0,		0,			0,	0,	DRIV_READY,	PID);	//turn right 25 deg
	...
	auto(40,	90,		0,		lPre(STS),	0,	0,	TIME_LIMIT,	1200);	//Forward
	auto(40,	90,		0,		lPre(STS),	OUT,0,	TIME_LIMIT,	400);	//dump
	auto(-90,	REV,	0,		0,			0,	0,	TIME_LIMIT,	700);	//Reverse
	auto(stopped(),				lPre(GND),	0,	0,	TIME_LIMIT,	1300);	//Drop lift
	auto(straight(REV),	0,		lPre(GND),	0,	0,	TIME_LIMIT,	1100);	//Reverse + drop lift
	auto(gyro2(35),		0,		lPre(BAR),	0,	0,	DRIV_READY,	PID);	//turn right 35 deg + lift
