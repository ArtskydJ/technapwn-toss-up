void autoTestThings(void)
	...
	auto(gyro2(900),	0,		0,		0,	0,	DRIV_READY,	PID);
	auto(gyro2(900),	0,		0,		0,	0,	DRIV_READY,	PID);
	auto(enc(1450,1450),0,		L_STSH,	0,	0,	FULL_READY,	PID);
	auto(straight(0),	usL(18),L_STSH,	0,	0,	FULL_READY,	PID);
...
void autoTestGyro(void)
	...
	autoResetStart(0, AUTON, 0, 0, 0);
		//L,	R,		S,		Lift,	Intk,Cata,End Type,	Other
	auto(gyro2(-900),	0,		0,		0,	0,	DRIV_READY,	PID);
	auto(straight(FWD),	0,		0,		0,	0,	TIME_LIMIT,	1000);
//	auto(stopped(),				0,		0,	0,	TIME_LIMIT,	1000,	0,	PID);
//	auto(gyro2(900),	0,		0,		0,	0,	DRIV_READY,	0,		0,	PID);
	autoResetEnd();
	}
