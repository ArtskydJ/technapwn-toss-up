void input()
...
#else //EMULATOR
	senLeftQSE.curr  += (float)mtrSlewed[DRIVE_BL1]*timerMaster/(127*2);//EMULATOR ONLY
	senRightQSE.curr += (float)mtrSlewed[DRIVE_BR1]*timerMaster/(127*2);//EMULATOR ONLY
#endif
	...
	if (joystickIsMoved(true) || nLCDButtons>0)
		timerRobotIdle = 0;
	if (timerRobotIdle>60000)
		timerRobotIdle=60000; //Prevent wrapping
#endif
